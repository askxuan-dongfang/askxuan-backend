...
const diyOrderRows = "id,order_no,user_id,design_id,material_fee,bless_fee,total_fee,status,payment_status,address_id,COALESCE(source,'') AS source,COALESCE(creator_id,'') AS creator_id,creator_share_rate,original_material_fee,price_changed,COALESCE(design_snapshot,'') AS design_snapshot,COALESCE(pricing_snapshot,'') AS pricing_snapshot,create_time,update_time"
