...
// 消费队列约定
const (
	QueueOrderPaymentNotify  = "order.payment.notify"
	QueueOrderLogisticsSync  = "order.logistics.sync"
)

// PaymentNotify 支付通知事件（与 payment-service 的 mq.PaymentNotify 对齐）
...
// LogisticsSync 物流状态变更事件（与 logistics-service 的 mq.LogisticsSync 对齐）
	...
	go func() {
		for {
			...
			if err := c.consumeLoop(ctx, bindings); err != nil {
				logx.Errorf("order 消费循环异常，3 秒后重连: %v", err)
				select {
				case <-ctx.Done():
					return
...
// parseLogisticsSync 解析物流同步
func parseLogisticsSync(body []byte) (LogisticsSync, error) {
