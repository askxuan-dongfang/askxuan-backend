...
func TestRewardsRouteAuthorization(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			svcCtx.Config.Auth.AccessSecret = "rewards-test-secret"
			registerRewards(server, svcCtx)
			req := httptest.NewRequest(http.MethodPost, "/api/v1/admin/marketing/rewards/campaigns", strings.NewReader(`{"name":""}`))
			req.Header.Set("Content-Type", "application/json")
			if tc.role != "" {
				token, err := common.GenAccessToken("rewards-test-secret", common.TokenInfo{UserId: 1, Roles: []string{tc.role}}, 3600)
				if err != nil {
					t.Fatal(err)
				}
...
func TestRewardCustomerIsolation(t *testing.T) {
	for _, tc := range []struct {
		name, role string
		code       int
	}{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			svcCtx.Config.Auth.AccessSecret = "rewards-test-secret"
			registerRewards(server, svcCtx)
			req := httptest.NewRequest(http.MethodPost, "/api/v1/marketing/rewards/campaigns/0/join", strings.NewReader(`{"name":""}`))
			req.Header.Set("Content-Type", "application/json")
			if tc.role != "" {
				token, err := common.GenAccessToken("rewards-test-secret", common.TokenInfo{UserId: 1, Roles: []string{tc.role}}, 3600)
				if err != nil {
