...
func (p *Producer) ensureChannel() error {
	...
	if err := ch.ExchangeDeclare(ExchangeAuditEvents, "fanout", true, false, false, false, nil); err != nil {
		_ = ch.Close()
		_ = conn.Close()
...
// PublishAuditResult 发布审核结果通知
func (p *Producer) PublishAuditResult(ctx context.Context, evt AuditResult) error {
	if evt.Time == "" {
	...
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.ch.PublishWithContext(ctx, ExchangeAuditEvents, "", false, false,
