...
const (
	...
	QueueLogisticsOrderShipped = "logistics.order.shipped"
)

// OrderShippedNotify 订单发货通知（与 order-service 的 OrderNotify 对齐）
	...
	go func() {
		for {
			...
			if err := c.consumeLoop(ctx, bindings); err != nil {
				logx.Errorf("logistics 消费循环异常，3 秒后重连: %v", err)
				select {
				case <-ctx.Done():
					return
...
// ParseOrderShipped 解析订单发货通知，仅返回 action=shipped 的事件
func ParseOrderShipped(body []byte) (OrderShippedNotify, bool) {
	var evt OrderShippedNotify
	if err := json.Unmarshal(body, &evt); err != nil {
		return OrderShippedNotify{}, false
	}
	if evt.Action != "shipped" {
		return OrderShippedNotify{}, false
	}
	return evt, true
}
