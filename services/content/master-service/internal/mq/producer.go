...
// 加持事件交换机（与 diy-service / temple-service 约定一致）
const (
	ExchangeBlessingEvents = "blessing.events"
)

// BlessingComplete 法师完成加持回传事件
...
func (p *Producer) ensureChannel() error {
	...
	if err := ch.ExchangeDeclare(ExchangeBlessingEvents, "fanout", true, false, false, false, nil); err != nil {
		_ = ch.Close()
		_ = conn.Close()
...
// PublishBlessingComplete 发布加持完成事件
func (p *Producer) PublishBlessingComplete(ctx context.Context, evt BlessingComplete) error {
	evt.EventType = "blessing.complete"
	if evt.Time == "" {
