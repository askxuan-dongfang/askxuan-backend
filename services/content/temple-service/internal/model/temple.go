...
func (m *defaultTempleModel) Insert(ctx context.Context, data *Temple) (int64, error) {
	const query = `INSERT INTO ` + templeTable + ` (code, name, region, type, sect, status, address, cover_image, rating, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
...
func (m *defaultTempleModel) FindOne(ctx context.Context, code string) (*Temple, error) {
	var t Temple
	query := fmt.Sprintf(`SELECT id, code, name, region, type, sect, status, address, cover_image, rating, description, create_time, update_time FROM %s WHERE code = ?`, templeTable)
...
func (m *defaultTempleModel) FindOneByPk(ctx context.Context, id int64) (*Temple, error) {
	var t Temple
	query := fmt.Sprintf(`SELECT id, code, name, region, type, sect, status, address, cover_image, rating, description, create_time, update_time FROM %s WHERE id = ?`, templeTable)
...
func (m *defaultTempleModel) FindList(ctx context.Context, filter TempleFilter, page, size int) ([]*Temple, int64, error) {
	...
	offset := (page - 1) * size
	listQuery := fmt.Sprintf(`SELECT id, code, name, region, type, sect, status, address, cover_image, rating, description, create_time, update_time FROM %s WHERE %s ORDER BY id ASC LIMIT ?, ?`,
		templeTable, where)
...
func (m *defaultTempleModel) Update(ctx context.Context, data *Temple) error {
	const query = `UPDATE ` + templeTable + ` SET name = ?, region = ?, type = ?, sect = ?, status = ?, address = ?, cover_image = ?, rating = ?, description = ? WHERE id = ?`
	_, err := m.conn.ExecCtx(ctx, query,
		data.Name, data.Region, data.Type, data.Sect, data.Status,
