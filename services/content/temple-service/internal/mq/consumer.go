...
// 消费队列约定
const (
	QueueTempleBlessingDispatch = "temple.blessing.dispatch"
	QueueTempleReviewNotify     = "temple.review.notify"
)
...
// ReviewNotify 新评价通知事件（与 review-service 的 mq.ReviewNotify 对齐）
...
// BlessingDispatch 加持任务派单事件（与 diy-service 的 mq.BlessingDispatch 对齐）
	...
	go func() {
		for {
			...
			if err := c.consumeLoop(ctx, bindings); err != nil {
				logx.Errorf("temple 消费循环异常，3 秒后重连: %v", err)
				select {
				case <-ctx.Done():
					return
...
// ParseBlessingDispatch 从消息体解析加持派单事件
// 仅处理 blessing.dispatch（eventType 为空），跳过 assign/complete
func ParseBlessingDispatch(body []byte) (BlessingDispatch, bool) {
	var probe struct {
		EventType string `json:"eventType"`
	}
	_ = json.Unmarshal(body, &probe)
	// blessing.dispatch 没有 EventType 字段，所以为空
	if probe.EventType != "" {
		return BlessingDispatch{}, false
	}
	var evt BlessingDispatch
