...
func (p *Producer) ensureChannel() error {
	...
	if err := ch.ExchangeDeclare(ExchangeReviewEvents, "fanout", true, false, false, false, nil); err != nil {
		_ = ch.Close()
		_ = conn.Close()
...
// PublishReviewNotify 发布新评价通知
func (p *Producer) PublishReviewNotify(ctx context.Context, evt ReviewNotify) error {
	if evt.Time == "" {
	...
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.ch.PublishWithContext(ctx, ExchangeReviewEvents, "", false, false,
