#!/usr/bin/env bash
# Rebuild only the product service from a Git archive; no database migrations.
# Usage: deploy-product.sh RELEASE BACKEND_SHA
set -euo pipefail
umask 077
...
tar -czf "$backup/product-source-before.tar.gz" -C "$base/backend" services/commerce/product-service
...
c=json.loads(subprocess.check_output(['docker','inspect','askxuan-product-service']))[0]
oldtag='askxuan/product-service:before-'+release
subprocess.run(['docker','tag',c['Image'],oldtag],check=True)
host=c['HostConfig']; cfg=c['Config']; networks={};volumes={}
d={'container_name':'askxuan-product-service','image':'askxuan/product-service:'+release,'restart':host['RestartPolicy']['Name'] or 'unless-stopped','environment':dict(v.split('=',1) for v in cfg['Env']),'volumes':[],'networks':{}}
...
for network,detail in c['NetworkSettings']['Networks'].items():
 ...
 d['networks'][network]={'aliases':list(dict.fromkeys(['product-service']+(detail.get('Aliases') or [])))}
...
for path,definition in [(Path(candidate)/'compose.release.json',d),(Path(backup)/'compose.rollback.json',{**d,'image':oldtag})]:
 path.write_text(json.dumps({'services':{'product-service':definition},'networks':networks,'volumes':volumes}))
(Path(backup)/'image-before.txt').write_text(c['Image']+'\n')
PY
docker compose -p askxuan -f "$candidate/compose.release.json" config --quiet
docker build -f "$candidate/backend/build/docker/Dockerfile" --build-arg SERVICE=services/commerce/product-service --build-arg BINARY=product -t "askxuan/product-service:$release" "$candidate/backend" > "$candidate/build-product.log" 2>&1
echo BUILT_PRODUCT
rollback() {
 trap - ERR
 docker compose -p askxuan -f "$backup/compose.rollback.json" up -d --no-build --no-deps product-service
 tar -xzf "$backup/product-source-before.tar.gz" -C "$base/backend"
 echo ROLLED_BACK_PRODUCT >&2
}
trap rollback ERR
docker compose -p askxuan -f "$candidate/compose.release.json" up -d --no-build --no-deps product-service
healthy=false
for attempt in $(seq 1 60); do
 if [[ "$(docker inspect askxuan-product-service --format '{{.State.Health.Status}}')" == healthy ]];then healthy=true;break;fi
 sleep 2
done
"$healthy"
curl -fsS http://127.0.0.1:8080/api/v1/health > "$candidate/gateway-health.json"
tar -xzf "$base/runtime/askxuan-backend-$backend_sha.tar.gz" -C "$base/backend" services/commerce/product-service scripts/ops/deploy-product.sh
docker tag "askxuan/product-service:$release" askxuan/product-service:local
printf 'product_service=%s\nrelease=%s\n' "$backend_sha" "$release" > "$candidate/release.txt"
touch "$candidate/DEPLOYED"
trap - ERR
echo "DEPLOYED_PRODUCT $release"
