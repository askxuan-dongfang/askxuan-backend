#!/usr/bin/env bash
# Rebuild only the AI service from a Git archive; no database migrations.
# Usage: deploy-ai.sh RELEASE BACKEND_SHA
set -euo pipefail
umask 077
...
tar -czf "$backup/ai-source-before.tar.gz" -C "$base/backend" services/infrastructure/ai-service
...
c=json.loads(subprocess.check_output(['docker','inspect','askxuan-ai-service']))[0]
oldtag='askxuan/ai-service:before-'+release
subprocess.run(['docker','tag',c['Image'],oldtag],check=True)
host=c['HostConfig']; cfg=c['Config']; networks={};volumes={}
d={'container_name':'askxuan-ai-service','image':'askxuan/ai-service:'+release,'restart':host['RestartPolicy']['Name'] or 'unless-stopped','environment':dict(v.split('=',1) for v in cfg['Env']),'volumes':[],'networks':{}}
...
for network,detail in c['NetworkSettings']['Networks'].items():
 ...
 d['networks'][network]={'aliases':list(dict.fromkeys(['ai-service']+(detail.get('Aliases') or [])))}
...
for path,definition in [(Path(candidate)/'compose.release.json',d),(Path(backup)/'compose.rollback.json',{**d,'image':oldtag})]:
 path.write_text(json.dumps({'services':{'ai-service':definition},'networks':networks,'volumes':volumes}))
(Path(backup)/'image-before.txt').write_text(c['Image']+'\n')
PY
docker compose -p askxuan -f "$candidate/compose.release.json" config --quiet
docker build -f "$candidate/backend/build/docker/Dockerfile" --build-arg SERVICE=services/infrastructure/ai-service --build-arg BINARY=ai -t "askxuan/ai-service:$release" "$candidate/backend" > "$candidate/build-ai.log" 2>&1
echo BUILT_AI
rollback() {
 trap - ERR
 docker compose -p askxuan -f "$backup/compose.rollback.json" up -d --no-build --no-deps ai-service
 tar -xzf "$backup/ai-source-before.tar.gz" -C "$base/backend"
 echo ROLLED_BACK_AI >&2
}
trap rollback ERR
docker compose -p askxuan -f "$candidate/compose.release.json" up -d --no-build --no-deps ai-service
healthy=false
for attempt in $(seq 1 60); do
 if [[ "$(docker inspect askxuan-ai-service --format '{{.State.Health.Status}}')" == healthy ]];then healthy=true;break;fi
 sleep 2
done
"$healthy"
curl -fsS http://127.0.0.1:8080/api/v1/health > "$candidate/gateway-health.json"
tar -xzf "$base/runtime/askxuan-backend-$backend_sha.tar.gz" -C "$base/backend" services/infrastructure/ai-service scripts/ops/deploy-ai.sh
docker tag "askxuan/ai-service:$release" askxuan/ai-service:local
printf 'ai_service=%s\nrelease=%s\n' "$backend_sha" "$release" > "$candidate/release.txt"
touch "$candidate/DEPLOYED"
trap - ERR
echo "DEPLOYED_AI $release"
