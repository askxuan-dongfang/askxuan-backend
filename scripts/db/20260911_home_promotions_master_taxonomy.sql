...
SELECT 'W005','明觉居士（演示）','演示资料','','独立居士','tibetan_buddhism','格鲁派','藏传佛教','已认证','on_shelf','normal','platform','藏文化交流,静心禅修','',4.8,1,29,72,30
WHERE NOT EXISTS(SELECT 1 FROM master WHERE code='W005');
INSERT INTO master (code,dharma_name,lay_name,temple_code,position,belief_code,sect,type,auth_status,shelf_status,platform_status,manage_by,specialties,avatar,rating,consult_enabled,consult_fee,consult_valid_hours,consult_response_minutes)
SELECT 'W006','守礼先生（演示）','演示资料','','民俗文化顾问','folk','民俗文化','民间信仰','已认证','on_shelf','normal','platform','传统礼俗,民俗祈愿','',4.7,1,19,72,30
...
SELECT 'W005','S001',68,'enabled' WHERE EXISTS(SELECT 1 FROM master WHERE code='W005' AND dharma_name='明觉居士（演示）') AND NOT EXISTS(SELECT 1 FROM master_service_tag WHERE master_code='W005' AND service_code='S001');
INSERT INTO master_service_tag(master_code,service_code,price,status)
SELECT 'W005','S002',48,'enabled' WHERE EXISTS(SELECT 1 FROM master WHERE code='W005' AND dharma_name='明觉居士（演示）') AND NOT EXISTS(SELECT 1 FROM master_service_tag WHERE master_code='W005' AND service_code='S002');
INSERT INTO master_service_tag(master_code,service_code,price,status)
SELECT 'W006','S001',58,'enabled' WHERE EXISTS(SELECT 1 FROM master WHERE code='W006' AND dharma_name='守礼先生（演示）') AND NOT EXISTS(SELECT 1 FROM master_service_tag WHERE master_code='W006' AND service_code='S001');
INSERT INTO master_service_tag(master_code,service_code,price,status)
SELECT 'W006','S010',88,'enabled' WHERE EXISTS(SELECT 1 FROM master WHERE code='W006' AND dharma_name='守礼先生（演示）') AND NOT EXISTS(SELECT 1 FROM master_service_tag WHERE master_code='W006' AND service_code='S010');
