...
[[ "$(jq '[.data.list[] | select(.resourceType == "product")] | length' <<<"$response")" -gt 0 ]]
[[ "$(jq '[.data.list[] | select(.resourceType == "service")] | length' <<<"$response")" -gt 0 ]]
[[ "$(jq '[.data.list[] | select((.sourceId | length) == 0 or .price < 0 or (.image | length) == 0 or (.orderTarget | length) == 0)] | length' <<<"$response")" == "0" ]]
[[ "$(jq '[.data.list[] | select(.resourceType == "product" and (.orderTarget | startswith("product:") | not))] | length' <<<"$response")" == "0" ]]
...
echo "PASS intention closed loop: resources=$(jq -r '.data.total' <<<"$response") mixed=product,service"
